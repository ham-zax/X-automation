...
This document describes the default preferences and technical universe for `@ham_zax`. The runtime source of truth is the persisted Growth Focus profile; `strategy.js` supplies defaults plus schema/normalization. `docs/OWNER_PROFILE_EVIDENCE.md` supplies owner-grounded strategic evidence that can justify durable expansion of that profile. Preferred content groups and broader audience/scope groups can be added, removed, renamed, reweighted, or disabled without code changes. Preferred groups bias selection; they do not hard-block a strong unregistered technical opportunity.
