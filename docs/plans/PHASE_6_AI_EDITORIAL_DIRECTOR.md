...
**Implementation status (2026-08-20): Implemented on `main`.** Canonical source refresh/observations, controlled research, persisted editorial planning, provider-independent AI passes, human selection, writer evidence, Today/Discover product surfaces, outcome provenance, and optional default-off background plan refresh are landed. Human approval/publication/send authority is unchanged. The checklist below is retained as the historical execution contract; this status line is authoritative for current implementation state.
