...
12. require the normal explicit human factuality/evidence confirmations and dashboard approval before a main-feed text draft becomes compatibility `ready`, except that a currently running `live` First-1,000 mission may approve only Original / Quote / Thread through the dedicated mission-agent authority using concrete stored verification provenance and `humanApprovedAt=null`; replies and Reposts remain outside that authority;
