...
12. for Engage Next, keep the ordinary human-reviewed exact-text lane available. Delegated autonomous evaluation may operate without per-reply approval, but live AI reply mutation remains unavailable unless its own compliant transport and X policy prerequisites are satisfied;
...
- Never automate authored Reply / Quote / Original / Thread mutation through the x.com UI. Authenticated browser access is read-only for automation. Exact outbound content must first pass repository authority/content gates, and automated mutation uses only the compliant official X API transport when its route is supported.
